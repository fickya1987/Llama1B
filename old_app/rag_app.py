...
if prompt := st.chat_input():
    if model_choice == 'Llama 3.2 : 1B':
        ...
        st.session_state.messages_deepseek_v3.append({"role": "user", "content": prompt})
        with st.chat_message("user"):
            st.write(prompt)
    elif model_choice == 'DeepSeek-R1':
        ...
        with st.spinner("Thinking..."):
            if uploaded_file is None:
                stream = client.chat.completions.create(
                    ...
                    messages=st.session_state.messages_deepseek_v3,
                    max_tokens=max_length,
                    temperature=temperature,
                    top_p=top_p,
                    stream=True
                ...
                if full_response.strip():
                    message = {"role": "assistant", "content": full_response}
                    st.session_state.messages_deepseek_v3.append(message)
                ...
                full_response = response["result"]
                st.write(full_response)
                st.session_state.messages_deepseek_v3.append({"role": "system", "content": 'Use the document as context to answer the questions. Answer the question in two-three lines. Do not respond with anything else. Only the Answer.'})
                st.session_state.messages_deepseek_v3.append({"role": "assistant", "content": full_response})
